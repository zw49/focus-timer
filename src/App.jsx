...
function App() {
  ...
  const presetTimes = Array.from({ length: 15 }, (_, i) => i + 1)
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-base-200">
      <div
        className={`flex items-center justify-center w-full relative transition-all duration-700 ${
          isRunning ? 'flex-8' : 'flex-1'
        }`}
      >
        {/* Focus circle */}
        <div
          className={`absolute card bg-base-100 shadow-xl rounded-full w-lg h-128 flex items-center justify-center transition-all duration-700 ease-out ${
            isRunning
        ...
        {/* Streak display */}
        <div
          className={`absolute flex flex-col items-center gap-4 transition-all duration-500 ease-out ${
            isRunning
          ...
          <div className="flex gap-2 justify-center">
            {last7Days.map((day) => (
              <div key={day} className="flex flex-col items-center gap-1">
                <div
                  className={`w-8 h-8 rounded flex items-center justify-center transition-colors duration-300 ${
      ...
      <div
        className={`flex-1 flex justify-center w-full transition-all duration-700 ease-out ${
          isRunning ? 'items-end pb-8' : 'items-start pt-8'
        }`}
      >
        <div className="card bg-base-100 shadow-xl p-4 transition-all duration-500">
          ...
          <div
            className={`overflow-hidden transition-all duration-500 ease-out ${
              isRunning ? 'max-h-0 opacity-0 mb-0' : 'max-h-40 opacity-100 mb-3'
            }`}
          >
              ...
              {presetTimes.map((mins) => (
                <button
                  ...
                  className={`btn btn-xs transition-all duration-200 ${
                    selectedMinutes === mins ? 'btn-primary' : 'btn-outline'
                  }`}
          ...
          <div className="flex gap-2 justify-center">
            <button
              className={`btn btn-lg transition-all duration-300 ${
                isRunning ? 'btn-ghost' : 'btn-primary'
              }`}
